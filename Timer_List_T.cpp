...
template <class TYPE, class FUNCTOR, class ACE_LOCK, typename TIME_POLICY>
void ACE_Timer_List_Iterator_T<TYPE, FUNCTOR, ACE_LOCK, TIME_POLICY>::first(void)
{
...
template <class TYPE, class FUNCTOR, class ACE_LOCK, typename TIME_POLICY>
void ACE_Timer_List_Iterator_T<TYPE, FUNCTOR, ACE_LOCK, TIME_POLICY>::next(void)
{
...
template <class TYPE, class FUNCTOR, class ACE_LOCK, typename TIME_POLICY>
bool ACE_Timer_List_Iterator_T<TYPE, FUNCTOR, ACE_LOCK, TIME_POLICY>::isdone(void) const
{
...
ACE_Timer_List_T<TYPE, FUNCTOR, ACE_LOCK, TIME_POLICY>::ACE_Timer_List_T(
    ...
    : Base_Timer_Queue(uf, fl, time_policy), head_(new ACE_Timer_Node_T<TYPE>), id_counter_(0)
{
    ACE_TRACE("ACE_Timer_List_T::ACE_Timer_List_T");

//** only have one element, so head_'s next is head_, prev is head_
...
template <class TYPE, class FUNCTOR, class ACE_LOCK, typename TIME_POLICY>
bool ACE_Timer_List_T<TYPE, FUNCTOR, ACE_LOCK, TIME_POLICY>::is_empty(void) const
{
...
ACE_Timer_List_T<TYPE, FUNCTOR, ACE_LOCK, TIME_POLICY>::~ACE_Timer_List_T(void)
{
    ...
    //!!! head_ is a dummy node!!!
    delete this->head_;
}

//remove all the nodes
template <class TYPE, class FUNCTOR, class ACE_LOCK, typename TIME_POLICY>
int ACE_Timer_List_T<TYPE, FUNCTOR, ACE_LOCK, TIME_POLICY>::close(void)
{
...
template <class TYPE, class FUNCTOR, class ACE_LOCK, typename TIME_POLICY>
void ACE_Timer_List_T<TYPE, FUNCTOR, ACE_LOCK, TIME_POLICY>::dump(void) const
{
...
template <class TYPE, class FUNCTOR, class ACE_LOCK, typename TIME_POLICY>
long ACE_Timer_List_T<TYPE, FUNCTOR, ACE_LOCK, TIME_POLICY>::schedule_i(
    const TYPE &type, const void *act, 
    const ACE_Time_Value &future_time, const ACE_Time_Value &interval)
{
...
template <class TYPE, class FUNCTOR, class ACE_LOCK, typename TIME_POLICY>
void ACE_Timer_List_T<TYPE, FUNCTOR, ACE_LOCK, TIME_POLICY>::schedule_i(
    ACE_Timer_Node_T<TYPE> *n, const ACE_Time_Value &expire)
{
    //** only when we schedule the first timer
    //** looks like a 环形队列
    if (this->is_empty())
    {
    ...
    long timer_id, const void **act, int skip_close)
{
    ...
    if (n != 0)
    {
        ...
        this->upcall_functor().cancel_type(
            *this, n->get_type(), skip_close, cookie);

        // cancel_timer() called once per <timer>.
        this->upcall_functor().cancel_timer(
            *this, n->get_type(), skip_close, cookie);
...
template <class TYPE, class FUNCTOR, class ACE_LOCK, typename TIME_POLICY>
int ACE_Timer_List_T<TYPE, FUNCTOR, ACE_LOCK, TIME_POLICY>::cancel(const TYPE &type, int skip_close)
{
    ACE_TRACE("ACE_Timer_List_T::cancel");
