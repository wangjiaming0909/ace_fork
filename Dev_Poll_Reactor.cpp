...
ACE_Dev_Poll_Reactor_Notify::ACE_Dev_Poll_Reactor_Notify(void)
    : dp_reactor_(0), notification_pipe_(), max_notify_iterations_(-1)
...
int ACE_Dev_Poll_Reactor_Notify::notify(ACE_Event_Handler *eh,
...
int ACE_Dev_Poll_Reactor_Notify::is_dispatchable(ACE_Notification_Buffer &)
{
...
int ACE_Dev_Poll_Reactor_Notify::purge_pending_notifications(
...
int ACE_Dev_Poll_Reactor::Handler_Repository::bind(
    ACE_HANDLE handle,
...
int ACE_Dev_Poll_Reactor::Handler_Repository::unbind(
    ACE_HANDLE handle, bool decr_refcnt)
{
...
ACE_Dev_Poll_Reactor::ACE_Dev_Poll_Reactor(ACE_Sig_Handler *sh,
                                           ...
                                           int mask_signals,
                                           int s_queue)
    : initialized_(false), poll_fd_(ACE_INVALID_HANDLE)
// , ready_set_ ()
#if defined(ACE_HAS_DEV_POLL)
      ,
      dp_fds_(0), start_pfds_(0), end_pfds_(0)
#endif /* ACE_HAS_DEV_POLL */
      ,
      token_(*this, s_queue), lock_adapter_(token_), deactivated_(0), timer_queue_(0), delete_timer_queue_(false), signal_handler_(0), delete_signal_handler_(false), notify_handler_(0), delete_notify_handler_(false), mask_signals_(mask_signals), restart_(0)
{
                               ...
                               ACE_Reactor_Notify *notify)
{
    ...
    // /dev/poll device has been fully initialized.
             ...
             (disable_notify_pipe == 0 && this->register_handler_i(
...
int ACE_Dev_Poll_Reactor::current_info(ACE_HANDLE, size_t & /* size */)
{
    ACE_NOTSUP_RETURN(-1);
}
...
int ACE_Dev_Poll_Reactor::work_pending(const ACE_Time_Value &max_wait_time)
{
...
int ACE_Dev_Poll_Reactor::work_pending_i(ACE_Time_Value *max_wait_time)
{
    ...
    int const timers_pending =
        ((this_timeout != 0 && max_wait_time == 0) || (this_timeout != 0 && max_wait_time != 0 && *this_timeout != *max_wait_time) ? 1 : 0);
...
int ACE_Dev_Poll_Reactor::register_handler(ACE_HANDLE handle,
                                           ACE_Event_Handler *event_handler,
                                           ACE_Reactor_Mask mask)
{
    ACE_TRACE("ACE_Dev_Poll_Reactor::register_handler");
    ...
    ACE_HANDLE handle, ACE_Event_Handler *event_handler, ACE_Reactor_Mask mask)
{
    ...
    if (handle == ACE_INVALID_HANDLE || mask == ACE_Event_Handler::NULL_MASK)
    {
        errno = EINVAL;
        return -1;
    }
...
int ACE_Dev_Poll_Reactor::register_handler(
...
int ACE_Dev_Poll_Reactor::register_handler(const ACE_Handle_Set &handle_set,
                                           ACE_Event_Handler *event_handler,
                                           ACE_Reactor_Mask mask)
{
    ACE_TRACE("ACE_Dev_Poll_Reactor::register_handler");
...
int ACE_Dev_Poll_Reactor::remove_handler(ACE_Event_Handler *handler,
                                         ACE_Reactor_Mask mask)
{
...
int ACE_Dev_Poll_Reactor::cancel_timer(ACE_Event_Handler *handler,
...
int ACE_Dev_Poll_Reactor::schedule_wakeup(ACE_Event_Handler *eh,
                                          ACE_Reactor_Mask mask)
{
...
int ACE_Dev_Poll_Reactor::cancel_wakeup(ACE_Event_Handler *eh,
                                        ACE_Reactor_Mask mask)
{
...
int ACE_Dev_Poll_Reactor::notify(ACE_Event_Handler *eh,
                                 ACE_Reactor_Mask mask,
                                 ACE_Time_Value *timeout)
{
...
int ACE_Dev_Poll_Reactor::purge_pending_notifications(ACE_Event_Handler *eh,
                                                      ACE_Reactor_Mask mask)
{
    if (this->notify_handler_ == 0)
        return 0;
                                  ...
                                  ACE_Event_Handler **event_handler)
{
    ...
    if (info != 0 && ACE_BIT_CMP_MASK(info->mask,
...
short ACE_Dev_Poll_Reactor::reactor_mask_to_poll_event(ACE_Reactor_Mask mask)
{
    ...
    // read set.
    if (ACE_BIT_ENABLED(mask, ACE_Event_Handler::READ_MASK) || ACE_BIT_ENABLED(mask, ACE_Event_Handler::ACCEPT_MASK) || ACE_BIT_ENABLED(mask, ACE_Event_Handler::CONNECT_MASK))
    {
#if defined(ACE_HAS_EVENT_POLL)
    ...
    if (ACE_BIT_ENABLED(mask, ACE_Event_Handler::WRITE_MASK) || ACE_BIT_ENABLED(mask, ACE_Event_Handler::CONNECT_MASK))
    {
#if defined(ACE_HAS_EVENT_POLL)
...
int ACE_Dev_Poll_Reactor::Token_Guard::acquire_quietly(ACE_Time_Value *max_wait)
{
...
int ACE_Dev_Poll_Reactor::Token_Guard::acquire(ACE_Time_Value *max_wait)
{
