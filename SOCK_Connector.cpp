                                    ...
                                    int reuse_addr)
{
    ...
    if (new_stream.get_handle() == ACE_INVALID_HANDLE && new_stream.open(SOCK_STREAM,
                                                                         protocol_family,
                                                                         protocol,
                                                                         reuse_addr) == -1)
        return -1;
...
int ACE_SOCK_Connector::shared_connect_finish(
    ACE_SOCK_Stream &new_stream, const ACE_Time_Value *timeout, int result)
{
                                ...
                                int /* perms */)
{
    ...
    if (this->shared_open(new_stream,
                          ...
                          reuse_addr) == -1)
        return -1;
    else if (this->shared_connect_start(new_stream, timeout, local_sap) == -1)
        return -1;
                                       ...
                                       int protocol)
{
    ...
    if (this->connect(new_stream,
                      ...
                      perms,
                      protocol) == -1 &&
        timeout != 0 && !(errno == EWOULDBLOCK || errno == ETIME || errno == ETIMEDOUT))
