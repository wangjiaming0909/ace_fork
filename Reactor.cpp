...
#if !defined(ACE_WIN32) || !defined(ACE_HAS_WINSOCK2) || (ACE_HAS_WINSOCK2 == 0) || defined(ACE_USE_SELECT_REACTOR_FOR_REACTOR_IMPL) || defined(ACE_USE_TP_REACTOR_FOR_REACTOR_IMPL) || defined(ACE_USE_DEV_POLL_REACTOR_FOR_REACTOR_IMPL)
...
void ACE_Reactor::close_singleton(void)
{
...
int ACE_Reactor::check_reconfiguration(ACE_Reactor *)
{
...
int ACE_Reactor::run_alertable_reactor_event_loop(REACTOR_EVENT_HOOK eh)
{
                                        ...
                                        REACTOR_EVENT_HOOK eh)
{
    ...
    while (1)
    {
        int result = this->implementation_->handle_events(tv);//handle_events 只进行一次select 和 dispatch
...
int ACE_Reactor::notify(ACE_Event_Handler *event_handler,
                        ACE_Reactor_Mask mask,
                        ACE_Time_Value *tv)
{
