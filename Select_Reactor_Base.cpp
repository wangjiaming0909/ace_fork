...
#else  /* !ACE_WIN32 */
    if (handle < 0 || static_cast<size_type>(handle) >= this->event_handlers_.size())
#endif /* ACE_WIN32 */
    {
        errno = EINVAL;
        return true;
...
int ACE_Select_Reactor_Handler_Repository::unbind(
    ACE_HANDLE handle,
    map_type::iterator pos,
    ACE_Reactor_Mask mask)
{
    ...
    bool const has_any_wait_mask =
        (this->select_reactor_.wait_set_.rd_mask_.is_set(handle) || this->select_reactor_.wait_set_.wr_mask_.is_set(handle) || this->select_reactor_.wait_set_.ex_mask_.is_set(handle));
    bool const has_any_suspend_mask =
        (this->select_reactor_.suspend_set_.rd_mask_.is_set(handle) || this->select_reactor_.suspend_set_.wr_mask_.is_set(handle) || this->select_reactor_.suspend_set_.ex_mask_.is_set(handle));
...
bool ACE_Select_Reactor_Handler_Repository_Iterator::next(
...
ACE_Select_Reactor_Notify::ACE_Select_Reactor_Notify(void)
    : select_reactor_(0), max_notify_iterations_(-1)
{
}
...
int ACE_Select_Reactor_Notify::max_notify_iterations(void)
{
...
void ACE_Select_Reactor_Notify::dump(void) const
{
#if defined(ACE_HAS_DUMP)
    ...
    ACE_Reactor_Impl *r, ACE_Timer_Queue *, int disable_notify_pipe)
{
    ...
    if (disable_notify_pipe == 0)
    {
...
#if defined(ACE_LACKS_LISTEN) && defined(ACE_LACKS_SOCKETPAIR) && !defined(ACE_HAS_STREAM_PIPES)
        ...
        if (ACE::set_flags(this->notification_pipe_.read_handle(),
            ...
            return this->select_reactor_->register_handler(this->notification_pipe_.read_handle(),
                                                           this,
...
int ACE_Select_Reactor_Notify::close(void)
{
    ...
    ACE_Time_Value *timeout)
{
    ...
    //?write_handle 已经在 open 的时候conenct 到 read_handle 上了
    //发送给 write_handle, read_handle 就可以读了
    //然而, read_handle 已经注册到了reactor 上了, 直接会被reactor 分配 handle_input
    //虽然发送的是一个buffer,但是其实不是buffer,buffer包含了 event_handler 和其对应的事件类型 mask
    //哪里进行了读操作呢??
    ssize_t const n = ACE::send(
        this->notification_pipe_.write_handle(), (char *)&buffer, sizeof buffer, timeout);
    if (n == -1)
    {
        return -1;
    }
                                                      ...
                                                      ACE_Handle_Set &rd_mask)
{
    ...
    if (read_handle != ACE_INVALID_HANDLE && rd_mask.is_set(read_handle))
    {
...
int ACE_Select_Reactor_Notify::is_dispatchable(ACE_Notification_Buffer &buffer)
{
#if defined(ACE_HAS_REACTOR_NOTIFICATION_QUEUE)
...
int ACE_Select_Reactor_Notify::dispatch_notify(ACE_Notification_Buffer &buffer)
{
    int result = 0;
...
int ACE_Select_Reactor_Notify::handle_input(ACE_HANDLE handle)
{
...
int ACE_Select_Reactor_Impl::purge_pending_notifications(ACE_Event_Handler *eh,
                                                         ACE_Reactor_Mask mask)
{
