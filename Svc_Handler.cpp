...
template <typename PEER_STREAM, typename SYNCH_TRAITS>
void ACE_Svc_Handler<PEER_STREAM, SYNCH_TRAITS>::operator delete(void *, void *)
{
...
#if !defined(ACE_LACKS_PLACEMENT_OPERATOR_DELETE)
template <typename PEER_STREAM, typename SYNCH_TRAITS>
void ACE_Svc_Handler<PEER_STREAM, SYNCH_TRAITS>::operator delete(void *p,
                                                                 const ACE_nothrow_t &)throw()
{
...
template <typename PEER_STREAM, typename SYNCH_TRAITS>
void ACE_Svc_Handler<PEER_STREAM, SYNCH_TRAITS>::cleanup_hint(void **act_holder)
{
...
template <typename PEER_STREAM, typename SYNCH_TRAITS>
int ACE_Svc_Handler<PEER_STREAM, SYNCH_TRAITS>::handle_timeout(const ACE_Time_Value &,
                                                               const void *)
{
...
template <typename PEER_STREAM, typename SYNCH_TRAITS>
int ACE_Svc_Handler<PEER_STREAM, SYNCH_TRAITS>::info(ACE_TCHAR **, size_t) const
{
...
template <typename PEER_STREAM, typename SYNCH_TRAITS>
int ACE_Svc_Handler<PEER_STREAM, SYNCH_TRAITS>::idle(u_long flags)
{
    if (this->recycler())
...
template <typename PEER_STREAM, typename SYNCH_TRAITS>
int ACE_Svc_Handler<PEER_STREAM, SYNCH_TRAITS>::recycle(void *)
{
...
template <typename PEER_STREAM, typename SYNCH_TRAITS>
int ACE_Buffered_Svc_Handler<PEER_STREAM, SYNCH_TRAITS>::put(ACE_Message_Block *mb,
                                                             ACE_Time_Value *tv)
{
    ...
    else
    {
        ...
        if (this->current_buffer_size_ >= this->maximum_buffer_size_ || (this->timeoutp_ != 0 && this->next_timeout_ <= ACE_OS::gettimeofday()))
            return this->flush_i();
        else
            return 0;
    }
...
template <typename PEER_STREAM, typename SYNCH_TRAITS>
int ACE_Buffered_Svc_Handler<PEER_STREAM, SYNCH_TRAITS>::flush(void)
{
    ACE_GUARD_RETURN(typename SYNCH_TRAITS::MUTEX, m, this->msg_queue()->lock(), -1);
...
template <typename PEER_STREAM, typename SYNCH_TRAITS>
int ACE_Buffered_Svc_Handler<PEER_STREAM, SYNCH_TRAITS>::flush_i(void)
{
...
template <typename PEER_STREAM, typename SYNCH_TRAITS>
void ACE_Buffered_Svc_Handler<PEER_STREAM, SYNCH_TRAITS>::dump(void) const
{
#if defined(ACE_HAS_DUMP)
...
template <typename PEER_STREAM, typename SYNCH_TRAITS>
int ACE_Buffered_Svc_Handler<PEER_STREAM, SYNCH_TRAITS>::handle_timeout(const ACE_Time_Value &,
                                                                        const void *)
{
